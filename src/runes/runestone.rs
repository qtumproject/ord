...
impl Runestone {
  ...
  pub(crate) fn encipher(&self) -> ScriptBuf {
    ...
    for chunk in payload.chunks(qtum::blockdata::constants::MAX_SCRIPT_ELEMENT_SIZE) {
      let push: &qtum::script::PushBytes = chunk.try_into().unwrap();
...
#[cfg(test)]
mod tests {
  use {super::*, qtum::script::PushBytes};
